...
os.chdir("/home/rundeck/projects/RulesInterpreterApp")
    ...
    for item in data:
        if item.get('ActionType') == 'report':
            ...
            if report_type == 'config':
                ...
                result['printIndex'] = {
                    'template': item.get('TemplateId','443b3174-4255-19be-6c74-6733258561bf')
                ...
                result['printTable'].append({
                    ...
                    'template': item.get('TemplateId','1caea5b2-4642-04ec-bb7a-674ee63959c2'),
                ...
                result['printPie'].append({
                    ...
                    'tableName': item.get('AliasName'),
                    'tableTitle': item.get('IntegrationName'),
                    'template': item.get('TemplateId','d95e96de-968b-7622-3f83-674eec68dea1')
                    # 'template': 'defaultPie'
                })
                ...
                result['printLine'].append({
                    ...
                    'tableName': item.get('AliasName'),
                    'tableTitle': item.get('IntegrationName'),
                    'template': item.get('TemplateId','509524a2-8470-18eb-e40a-674eeaf9a5ef'),
                ...
                result['printBar'].append({
                    ...
                    'tableName': item.get('AliasName'),
                    'tableTitle': item.get('IntegrationName'),
                    'template': item.get('TemplateId','125d042c-f271-3d27-f881-674ee5a2bfda'),
                    # 'template': 'defaultBar',
                    'grid': False
                })
                ...
                result['printDonut'].append({
                    ...
                    'tableName': item.get('AliasName'),
                    'tableTitle': item.get('IntegrationName'),
                    'template': item.get('TemplateId','29f25f08-37da-141e-d1f4-674ee8acbc82'),
...
def printCoverPage(config):
    ...
    coverTemplate = config['template'] if 'template' in config and config['template'] != None else '4d9bc55a-36e0-3cc2-fe05-674ee7278519'
...
def printTable(data, config, start_page):
    ...
    templateContent = retrieveHTMLTemplate(config['template']).replace(
                'id="icon"', 'src="file:///home/rundeck/projects/RulesInterpreterApp/images/logoFull.png"'
            )
    template = env.from_string(templateContent)
...
async def printPie(response, config, start_page):
    ...
    templateContent = retrieveHTMLTemplate(config['template']).replace(
                ...
                "id=\"graph\"", "src=\"file:///home/rundeck/projects/RulesInterpreterApp/images/charts/pieChart.png\""
            )
...
async def printLine(response, config, start_page):
    ...
    templateContent = retrieveHTMLTemplate(config['template']).replace(
                ...
                'id="graph"', 'src="file:///home/rundeck/projects/RulesInterpreterApp/images/charts/lineChart.png"'
            )
...
async def printBar(response, config, start_page):
    ...
    templateContent = retrieveHTMLTemplate(config['template']).replace(
                ...
                'id="graph"', 'src="file:///home/rundeck/projects/RulesInterpreterApp/images/charts/barChart.png"'
            )
...
async def printDonut(response, config, start_page):
    ...
    templateContent = retrieveHTMLTemplate(config['template']).replace(
                ...
                'id="graph"', 'src="file:///home/rundeck/projects/RulesInterpreterApp/images/charts/donutChart.png"'
            )
...
def printClose(config):
    ...
    template_loader = jinja2.FileSystemLoader('./')
    template_env = jinja2.Environment(loader=template_loader)
    indexTemplate = config['template'] if 'template' in config and config['template'] != None else '443b3174-4255-19be-6c74-6733258561bf'
